...
<body bgcolor="#ffffff">
<h1>場所変更ページ</h1>
<form name="add_place" method="POST" action="update.php">
...
if(!empty($_POST['id'])){
  ...
  $sql = 'update parkings set latitude=?, longitude=?, genre=?, name=?, outline=?, postalcode=?, address=?, phonenumber=?, opentime=?, closingday=?, price=?, remarks=?, link=? where id = ?';
  $prep = $dbh->prepare($sql); // プリペアードステートメントとして扱う
  $result = $prep->execute(array($_POST['latitude'], $_POST['longitude'], $_POST['genre'], $_POST['name'], $_POST['outline'], $_POST['postalcode'], $_POST['address'], $_POST['phonenumber'], $_POST['opentime'], $_POST['closingday'], $_POST['price'], $_POST['remarks'], $_POST['link'] ,$_POST['id'] )); // プリペアードステートメントは execute メソッドを使う。
